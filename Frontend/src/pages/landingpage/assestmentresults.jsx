...
const getResultInfo = (type, score) => {
  switch (type) {
    case 'phq9':
      if (score <= 4) return { level: 'Minimal', icon: CheckCircle, color: '#22c55e', bgColor: '#dcfce7',
      ...
      if (score <= 9) return { level: 'Mild', icon: Info, color: '#eab308', bgColor: '#fefce8',
      ...
      if (score <= 14) return { level: 'Moderate', icon: AlertTriangle, color: '#f97316', bgColor: '#fff7ed',
      ...
      if (score <= 19) return { level: 'Moderately Severe', icon: AlertCircle, color: '#ef4444', bgColor: '#fef2f2',
      ...
      return { level: 'Severe', icon: AlertCircle, color: '#dc2626', bgColor: '#fef2f2',
    ...
    case 'gad7':
      ...
      return { level: 'Severe', icon: AlertCircle, color: '#ef4444', bgColor: '#fef2f2',
    ...
    case 'ghq':
      if (score <= 2) return { level: 'Good', icon: CheckCircle, color: '#22c55e', bgColor: '#dcfce7',
      ...
      if (score <= 5) return { level: 'Mild Concerns', icon: Info, color: '#eab308', bgColor: '#fefce8',
      ...
      return { level: 'Significant Concerns', icon: AlertTriangle, color: '#f97316', bgColor: '#fff7ed',
    ...
    default:
      return { level: 'Unknown', icon: Info, color: '#6b7280', bgColor: '#f9fafb',
        description: 'Assessment completed.', recommendations: ['Thank you for completing the assessment'] };
  }
};

export function AssessmentResults({ assessmentType, score, onReturnToHub }) {
        ...
        <Card className="p-8 border" style={{ backgroundColor: 'white', borderColor: '#A8D0E6' }}>
          <div className="text-center mb-8">
            ...
            <div className="mb-4">
              ...
              <span className="text-lg ml-2" style={{ color: '#4A4A4A' }}>
                / {assessmentType === 'ghq' ? '12' : assessmentType === 'gad7' ? '21' : '27'}
              </span>
            </div>
          ...
          <div className="mb-8">
            <h3 className="mb-4" style={{ color: '#4A4A4A' }}>What This Means</h3>
            <p className="mb-6" style={{ color: '#4A4A4A' }}>{result.description}</p>
            <h3 className="mb-4" style={{ color: '#4A4A4A' }}>Recommendations</h3>
            <ul className="space-y-2 mb-8">
              {result.recommendations.map((rec, index) => (
                <li key={index} className="flex items-start gap-2">
                  <div className="w-2 h-2 rounded-full mt-2 flex-shrink-0"
                       style={{ backgroundColor: '#7B9ACC' }}></div>
          ...
          <div className="text-center space-y-4">
            <Button onClick={onReturnToHub} className="text-white" style={{ backgroundColor: '#7B9ACC' }}>
             ...
             (score >= 4 && assessmentType === 'ghq') ? (
              <div className="pt-4 border-t" style={{ borderColor: '#A8D0E6' }}>
                ...
                <div className="flex flex-col sm:flex-row gap-2 justify-center">
                  <Button variant="outline" className="border text-white"
